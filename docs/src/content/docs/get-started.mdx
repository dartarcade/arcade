...
Future<void> main() {
    Route.get('/hello', (context) => 'Hello World!');

    // Add the following route
    Route.get('/hello/:message', (context) {
        final message = context.pathParameters['message'];
        final time = DateTime.now().toIso8601String();
...
Now if we visit `http://localhost:8080/hello/Hello%20World` we should see the following JSON response:
